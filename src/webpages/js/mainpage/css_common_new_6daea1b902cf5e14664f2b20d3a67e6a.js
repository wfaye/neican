...
(function ($) {
    var ivGrid = function (elem, options) {
        ...
        this.config = {
            ...
            headerTemplate: '<div class="white-overlay">' + '<img style="margin-top:100px;" src="<~%=iv.util.getThemedImageUrl(\'LoadingAnimation\')%~>">' + "<h1>Loading...</h1>" + "</div>" + '<div class="rows-header">' + '<~% if (className != "") { %~> ' + '	<div class="fillerLeft"></div>' + "<~% } %~>" + "<~% _.each(colConfigs, function(col) { %~>" + '	<~% if(typeof col.sortable != "undefined" && col.sortable == false) { %~>' + '		<div class="not-sortable" name="<~%=col.name%~>" style="float:left;width:<~%=col.widthInPercent%~>%;"><span class="header-text" style="float:left;"><~%=col.displayName%~></span></div>' + "	<~% } else { %~>" + '		<div class="sortable-column"  name="<~%=col.name%~>" style="float:left;width:<~%=col.widthInPercent%~>%;"><span class="header-text" style="float:left;"><~%=col.displayName%~></span><span class="common_sprites"></span></div>' + "	<~% } %~>" + "<~%});%~>" + '<~% if (className != "") { %~> ' + '	<div class="fillerRight"></div>' + "<~% } %~>" + "</div>",
