...
function doAddOptionItem(frmElement, optName, optValue, defaultSelected, selected) {
    frmElement.options[frmElement.options.length] = new Option(optName, optValue, defaultSelected, selected);
}
...
function NameSorter(paramA, paramB) {
    ...
    if (paramA.name && paramB.name) {
        if (paramA.name == "All" || paramB.name == "All") {
            ...
            result = ((paramA.name < paramB.name) ? -1 : ((paramA.name > paramB.name) ? 1 : 0));
        }
    }
    return result;
}
...
function logThisClick(fromPage, clickedPoint) {
    setTimeout('logActivity("from=' + fromPage + "&clickedPoint=" + clickedPoint + '")', 1000);
}
function logActivity(params) {
    var jsessionid = "";
    if (params.indexOf(";jsessionid=") != -1) {
        var start = params.indexOf(";jsessionid=") + 12;
        var end = (params.indexOf("?") != -1) ? params.indexOf("?") : params.length;
        jsessionid = params.substring(start, end);
    } else {
        jsessionid = getCookie("JSESSIONID");
    }
    Spry.Utils.loadURL("GET", LOG_ACTIVITY + ";jsessionid=" + jsessionid + "?" + escape(params), true);
}
...
function doLaunchLinkedIn(companyName) {
    ...
    companyName = companyName.replace(/(\bASA\b)|(\bCompanies\b)|(\bCompany\b)|(\bGroup\b)|(\bGrp\b)|(\bGrupo\b)|(\bGruppo\b)|(\bHolding\b)|(\bHoldings\b)|(\/NY\b)|(\bIndustries\b)|(\bKoninklijke\b)|(\bN\.V\.)|(\bNV\b)|(\bPart\.)|(\bParticipacoes\b)|(\bSGPS\b)/gi, "");
    companyName = companyName.replace(/\s\s*/g, " ");
    companyName = companyName.replace(/(^\s*)|(\s*$)/g, "");
    companyName = '"' + companyName + '"';
    companyName = escape(companyName);
    URL = "http://www.linkedin.com/search?search=&company=" + companyName + "&currentCompany=co";
    ...
    setTimeout('logActivity("action=launchLinkedIn&companyName=' + escape(companyName) + '")', 1000);
...
function doLaunchLinkedInPerson(firstName, lastName, companyName) {
    ...
    companyName = '"' + companyName + '"';
    companyName = escape(companyName);
    URL = "http://www.linkedin.com/search?search=&lname=" + lastName + "&company=" + companyName + "&fname=" + firstName + "&currentCompany=cp";
    var name = "linkedInWindow";
    var attributes = "toolbar=yes,location=yes,directories=no,status=yes,menubar=yes,scrollbars=yes,resizable=yes,width=810,height=740";
    linkedInWindow = window.open(URL, name, attributes);
    setTimeout('logActivity("action=launchLinkedInPerson&companyName=' + escape(companyName) + "&personName=" + escape() + '")', 1000);
    linkedInWindow.focus();
}
function doLaunchJigsaw(lastName, firstName, companyName) {
    ...
    lastName = escape(lastName);
    companyName = escape(companyName);
    URL = "http://www.jigsaw.com/SearchAcrossCompanies.xhtml?opCode=search&cnName=" + lastName + "&cnFirstName=" + firstName + "&cmName=" + companyName;
...
function doLoadCustomRangeFields(frmElement, frmElementOldValue, frmObj, methodName) {
...
function refreshPage(parameterKey, parameterValue) {
    ...
    if (searchString != "") {
        if (searchString.indexOf("help=") != -1) {
            searchString = searchString.substring(0, searchString.indexOf("help=")) + parameter + searchString.substring(searchString.indexOf("help=") + 9, searchString.length);
        } else {
    ...
    if ((jsessionid_position != -1) && window.location.pathname.indexOf(";jsessionid=") == -1) {
        if (window.location.search) {
            jsessionid = window.location.href.substring(jsessionid_position, window.location.href.indexOf(window.location.search));
        } else {
            if (window.location.hash) {
                jsessionid = window.location.href.substring(jsessionid_position, window.location.href.indexOf(window.location.hash));
            } else {
                jsessionid = window.location.href.substring(jsessionid_position);
            }
        }
    }
    window.location.href = window.location.protocol + "//" + window.location.host + window.location.pathname + jsessionid + searchString + window.location.hash;
}
...
function doToggleImageOpen(imgObj, isOpen) {
    if (isOpen) {
        imgObj.src = imgObj.src.substring(0, imgObj.src.indexOf(".gif") - 2) + "_0.gif";
    } else {
        imgObj.src = imgObj.src.substring(0, imgObj.src.indexOf(".gif") - 2) + "_1.gif";
    }
}
...
var hs = {
    ...
    expanders: [],
    overrides: ["allowSizeReduction", "anchor", "align", "targetX", "targetY", "outlineType", "outlineWhileAnimating", "spaceForCaption", "captionId", "captionText", "captionEval", "transitions", "dimmingOpacity", "contentId", "width", "height", "allowWidthReduction", "allowHeightReduction", "preserveContent", "objectType", "cacheAjax", "objectWidth", "objectHeight", "objectLoadTime", "swfObject", "wrapperClassName", "minWidth", "minHeight", "slideshowGroup", "easing", "easingClose", "fadeInOut"],
    overlays: [],
    faders: [],
    pendingOutlines: {},
    sleeping: [],
    ...
    getPageSize: function () {
        ...
        var b = document.body;
        var xScroll = (window.innerWidth && window.scrollMaxX) ? window.innerWidth + window.scrollMaxX : (b.scrollWidth > b.offsetWidth ? b.scrollWidth : b.offsetWidth),
            yScroll = (window.innerHeight && window.scrollMaxY) ? window.innerHeight + window.scrollMaxY : (b.scrollHeight > b.offsetHeight ? b.scrollHeight : b.offsetHeight),
            pageWidth = hs.ie ? iebody.scrollWidth : (document.documentElement.clientWidth || self.innerWidth),
            pageHeight = hs.ie ? iebody.clientHeight : (document.documentElement.clientHeight || self.innerHeight);
    ...
    getSelfRendered: function () {
        var s = '<div class="highslide-header"><ul>' + '<li class="highslide-previous"><a onclick="return hs.previous(this)" href="#">' + hs.previousText + "</a></li>" + '<li class="highslide-next"><a onclick="return hs.next(this)" href="#">' + hs.nextText + "</a></li>" + '<li class="highslide-move"><a href="#" onclick="return false">' + hs.moveText + "</a></li>" + '<li class="highslide-close"><a onclick="return hs.close(this)" title="' + hs.closeTitle + '" href="#">' + hs.closeText + "</a></li>" + "</ul></div>" + '<div class="highslide-body"></div>' + '<div class="highslide-footer"><div>' + '<span class="highslide-resize" title="' + hs.resizeTitle + '"><span></span></span>' + "</div></div>";
    ...
    setClickEvents: function () {
        ...
        if (!hs.pageLoaded) {
            setTimeout(hs.setClickEvents, 50);
        }
    },
    ...
    unDim: function (key) {
        ...
        if (typeof key != "undefined" && key != hs.dimmer.owner) {
            return;
        }
        if (hs.upcoming && hs.getParam(hs.upcoming, "dimmingOpacity")) {
            return;
        }
        if (hs.geckoMac && hs.dimmingGeckoFix) {
            hs.dimmer.style.background = "none";
        } else {
            hs.fade(hs.dimmer, hs.dimmingOpacity, 0, hs.dimmingDuration);
        }
        setTimeout(function () {
    ...
    cleanUp: function () {
        for (var i = 0; i < hs.expanders.length;
        i++) {
            ...
            if (e.type == "mouseup") {
                ...
                if (hs.dragArgs) {
                    ...
                    if (!hasDragged && !hs.hasFocused && !/(move|resize)/.test(hs.dragArgs.type)) {
                        ...
                        if (hasDragged || (!hasDragged && hs.hasHtmlexpanders)) {
...
hs.Outline.prototype = {
    ...
    setPosition: function (exp, x, y, w, h, vis) {
        if (vis) {
            this.table.style.visibility = (h >= 4 * this.offset) ? "visible" : "hidden";
        }
...
hs.Expander = function (a, params, custom, contentType) {
    ...
    var el = this.thumb = ((params && params.thumbnailId) ? hs.$(params.thumbnailId) : null) || a.getElementsByTagName("img")[0] || a;
    ...
    this.wrapper.onmouseover = function (e) {
        try {
            hs.expanders[key].wrapperMouseHandler(e);
        } catch (e) {}
    };
...
hs.Expander.prototype = {
    ...
    htmlCreate: function () {
        ...
        this.tempContainer = hs.createElement("div", {
            className: this.wrapperClassName
        }, {
    ...
    contentLoaded: function () {
        try {
            ...
            this.x = {
                ...
                span: this.newWidth,
                minSpan: (this.newWidth < minWidth && !hs.padToMinWidth) ? this.newWidth : minWidth,
                justify: justify.x,
                target: this.targetX,
    ...
    writeExtendedContent: function (loadTime) {
        ...
        if (this.objectType == "iframe") {
            ...
            var onload = (hs.ie && this.objectLoadTime == "before") ? 'onload="if(hs.expanders[' + this.key + "])hs.expanders[" + this.key + '].contentLoaded();"' : "";
            var tag = hs.ie ? '<iframe src="javascript:false;" name="hsIframe' + this.key + '" ' + onload + "/>" : "iframe";
            this.iframe = hs.createElement(tag, {
                name: "hsIframe" + this.key,
                frameBorder: 0,
    ...
    justify: function (p) {
        ...
        if (p.target && hs.$(p.target)) {
            ...
            if (tgt && tgt[1] && tgt[1].match(/^[-]?[0-9]+px$/)) {
                p.min += parseInt(tgt[1]);
            }
        } else {
    ...
    changeSize: function (up, from, to, dur, steps) {
        var trans = this.transitions,
            t = (trans[1] && hs.upcoming && hs.getParam(hs.upcoming, "transitions")[1] == trans[1]) ? trans[1] : trans[0];
        if (this[t]) {
            this[t](up, from, to);
            return;
        }
        if (up && this.objOutline && !this.outlineWhileAnimating) {
            ...
            if (!up && this.objOutline) {
                if (this.outlineWhileAnimating) {
                    this.objOutline.setPosition(this, from.x, from.y, from.w, from.h);
                } else {
        ...
        if (up) {
            setTimeout(function () {
                if (exp.objOutline) {
                    exp.objOutline.table.style.visibility = "visible";
                }
            }, t);
            setTimeout(function () {
                if (exp.caption) {
    ...
    fade: function (up, from, to) {
        ...
        if (this.objOutline) {
            ...
            for (var i = from.o; dir * i <= dir * to.o; i += dir, t += 25) {
                (function () {
                    ...
                    setTimeout(function () {
                        exp.objOutline.setPosition(exp, (exp.x.min + o), (exp.y.min + o), (exp.x.span - 2 * o), (exp.y.span - 2 * o), 1, t);
                    }, t);
                })();
            }
        }
        if (up) {
            ...
            setTimeout(function () {
                if (exp.objOutline) {
                    exp.objOutline.destroy(exp.preserveContent);
                }
    ...
    setSize: function (to) {
        try {
            if (this.isHtml) {
                ...
                this.content.style.width = ((to.imgW && !isNaN(to.imgW)) ? to.imgW : to.w) + "px";
                if (hs.safari) {
    ...
    getCaption: function () {
        if (!hs.fireEvent(this, "onBeforeGetCaption")) {
            return;
        }
        ...
        if (!this.caption && this.captionText) {
            this.caption = hs.createElement("div", {
    ...
    writeCaption: function () {
        try {
            ...
            hs.setStyles(this.caption, {
                visibility: "hidden",
                marginTop: hs.safari ? 0 : "-" + this.y.span + "px"
            });
    ...
    placeCaption: function (height, end) {
        ...
        if (o) {
            ...
            if (o.hasAlphaImageLoader) {
                o.td[3].style.height = o.td[5].style.height = o.td[4].style.height;
            }
        }
        if (end) {
            this.prepareNextOutline();
                ...
                if (visibility == "visible" && hiddenBy) {
                    ...
                    if (visibility == "hidden") {
                        ...
                        if (!clearsX && !clearsY && wrapperKey != this.key) {
                            ...
                            if (hiddenBy == "[" + this.key + "]" || hs.focusKey == wrapperKey) {
    ...
    wrapperMouseHandler: function (e) {
        ...
        if (hs.getExpander(e.relatedTarget) == this || hs.dragArgs) {
            return;
        }
        if (this.isExpanded) {
            hs.fireEvent(this, over ? "onMouseOver" : "onMouseOut", e);
        }
        for (var i = 0; i < this.overlays.length; i++) {
...
hs.setClickEvents();
hs.graphicsDir = "/iv/common/highslide/graphics/";
hs.outlineType = "rounded-white";
hs.outlineWhileAnimating = true;
hs.showCredits = false;
hs.fullExpandTitle = "Please hang on while we load that.";
hs.restoreTitle = "Click to Restore";
hs.focusTitle = "Click to Focus Title";
